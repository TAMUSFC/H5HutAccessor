"""
    Utility class for dealing with H5Hut files generated by OPAL
"""
import os
...
class H5HutAccessor():
    """
    An accessor class that loads an H5Hut file and stores the data therein in 
    more useful numpy arrays

    Step attributes:
    
    t - Global time (sec)
    s - s coordinate of reference (pid=0) particle (m)
    theta - Angle of reference (pid=0) particle (deg)
    Bref, Eref - Fields seen at reference (pid=0) particle (T, MV/m)

    For all accessible attributes, each row corresponds to one step, columns 
    correspond to particle IDs:

    x, y, z - particle position in global coordinate system (m)
    px, py, pz - particle momentum (units of beta*gamma)
    
    """

    def __init__(self, fn, stride=1, minstep=0, maxstep=-1):
        """
        fn - path to the H5Hut file to process
        stride - stepsize between adjacent steps to load (useful if the user wants 
            to examine a very fine file without using ALL the temporal resolution)
        minstep - phase dump (not integrator!) step to begin processing from
        mmaxstep - maximum phase dump (not integrator!) step to process
        """
